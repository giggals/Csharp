...
class Program
{
    static void Main(string[] args)
    {
        using (SqlConnection connection = new SqlConnection(Configuration.ConnectionString))
        {
            ...
            string tableEvillnesFactors = "CREATE TABLE EvilnessFactors(Id INT PRIMARY KEY IDENTITY, Name VARCHAR(50))";

            string tablevillans = "CREATE TABLE Villains (Id INT PRIMARY KEY IDENTITY, Name VARCHAR(50), EvilnessFactorId INT FOREIGN KEY REFERENCES EvilnessFactors(Id))";

            string tableminionsVillans = "CREATE TABLE MinionsVillains (MinionId INT FOREIGN KEY REFERENCES Minions(Id),VillainId INT FOREIGN KEY REFERENCES Villains(Id),CONSTRAINT PK_MinionsVillains PRIMARY KEY (MinionId, VillainId))";

            ExecuteNonQuery(connection, tableCountries);
            ExecuteNonQuery(connection, tableTowns);
            ExecuteNonQuery(connection, tableMinions);
            ExecuteNonQuery(connection, tableEvillnesFactors);
            ExecuteNonQuery(connection, tablevillans);
            ExecuteNonQuery(connection, tableminionsVillans);
            ...
            string insertIntoMinionsVillans = "INSERT INTO MinionsVillains (MinionId, VillainId) VALUES (4,2),(1,1),(5,7),(3,5),(2,6),(11,5),(8,4),(9,7),(7,1),(1,3),(7,3),(5,3),(4,3),(1,2),(2,1),(2,7)"; ;

            ExecuteNonQuery(connection, insertIntoCountries);
            ExecuteNonQuery(connection, insertIntoTowns);
            ExecuteNonQuery(connection, insertIntoMinions);
            ExecuteNonQuery(connection, insertIntoEvilnessFactors);
            ExecuteNonQuery(connection, insertIntoVillains);
            ExecuteNonQuery(connection, insertIntoMinionsVillans);
